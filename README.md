...
```bash
python -m src.scripts.color_picker
```
...
Мобильная робототехническая система **Festo Robotino** имеет размеры: Диаметр $D=450 \, мм$ что является $\approx \frac{1}{5}$ от длины полигона, а высота $h=370 \, мм$. Управление данным роботм возможно осуществить при помощи среды Robotino View, однако в контексте данной работы ввиду работы с внешней камерой решено осуществлять коммуникацию через socket, и отправлять линейные скорости для движения на веб-сервис.
...
| Зелёный вектор | Вектор отталкивания от препятсивя (APF) |
...
На изображении можно увидеть, что проецирование зелёных контуров на поверхность происходит не совсем точно, однако параметры с котрыми происходит проецирование были подобраны эмпирически и именно с ними робот не задевал препятствия при размещении последних возле краёв полигона. 

В результате решения данной задачи был разработан алгоритм и написан программный код для движения робота из точки А в точку Б с избежанием препятствий на пути следования.
...
Карта с допустимыми точками пути формируется на основе дискретизованной маски препятствий с расширенными границами на радиус робота для избежания столкновений.
...
1. Целевая точка достигнута - робот стоит на месте.
