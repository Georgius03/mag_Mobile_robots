...
class SocketParams(BaseModel):
    ...
    enable: (bool | int) = 0                                                # Отк./вкл. для тестирования (0/1) чтобы не производилось подключение к роботу

# Параметры камеры
class CameraParams(BaseModel):
    online: (bool | int) = 0                                                # Переключение на подгрузку кадров из видео-записьи/трансляцитю с камеры (0/1)
...
class GridParams(BaseModel):
    ...
    robot_radius: int = 275                                                 # Радиус робота (+ допуск из-за искажений камеры при проецировании)
    WALL_WIDTH: int = Field(default=150, alias="WALL_WIDTH")                # Толщина стенки по контуру рабочего поля (для 1 задачи)
